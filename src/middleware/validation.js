...
export const schemas = {
  ...
  register: Joi.object({
    ...
    phone: Joi.string().trim().pattern(/^[+]?[(]?[0-9]{1,4}[)]?[-\s.]?[(]?[0-9]{1,4}[)]?[-\s.]?[0-9]{1,9}$/).required(),
  ...
  updateTalent: Joi.object({
    ...
    languages: Joi.array().items(Joi.string().trim().max(50)).optional(),
    specialties: Joi.array().items(Joi.string().trim().max(100)).optional(),
    removedImageIds: Joi.alternatives().try(
  ...
  updateHomeConfig: Joi.object({
    featuredTalentIds: Joi.alternatives().try(
      Joi.string(), // JSON string from FormData
      Joi.array().items(Joi.number().integer().positive()).length(4)
    ).required(),
    remainingDesktopImages: Joi.alternatives().try(
      Joi.string(), // JSON string from FormData
      Joi.array().items(Joi.string())
    ).optional(),
    remainingMobileImages: Joi.alternatives().try(
      Joi.string(), // JSON string from FormData
      Joi.array().items(Joi.string())
    ).optional()
  })
